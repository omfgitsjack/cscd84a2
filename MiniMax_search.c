 ...
 */
	...
	int distFromCat = abs(mouse[0][0] - cat_loc[0][0]) + abs(mouse[0][1] - cat_loc[0][1]);
	int distFromCheese = abs(mouse[0][0] - cheese_loc[0][0]) + abs(mouse[0][1] - cheese_loc[0][1]);

	if (distFromCat > distFromCheese) return -1;
	if (distFromCheese > distFromCat) return 1;
